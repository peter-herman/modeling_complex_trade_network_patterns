* 2006 partial gravity analysis for comparrison with ERGM
* DESCRIPTION: This condcts a probit gravity analysis using the partial sample 
* of the top 50 countries and trade in HS chapter 51 for use in the model comparrisons.
* log close
clear all
set matsize 8000
set maxvar 30000
...
log using "2006_partial_gravity_log_hs51.log", replace
set more off

local use_year 2006

import delimited "comparisons\HS51\partial_data_set_hs51_top50_with_ntw_vars.csv", varnames(1) clear
...
export delimited exporter importer year traded phat_standard phat_bilat_ntwk phat_all_ntwk using "2006_probit_predicted_probabilities_subset_network_stats_hs51", replace


esttab probit_simple probit_network probit_network_noFE using 2006_partial_probit_stata_output.txt, se aic pr2 scalars(ll) star(* 0.1 ** 0.05 *** 0.01) replace drop(exp_fe* imp_fe*) compress
